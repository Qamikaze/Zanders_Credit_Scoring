...
def get_data(small):
    train = pd.read_csv('train_data.csv.gz',  header=0, sep=',', quotechar='"')
    test = pd.read_csv('test_data.csv.gz',  header=0, sep=',', quotechar='"')
...
from Booster import XGBModelClassifier as XGB
model = XGB(n_estimators=3, #learning_rate=0.1, \
